...
"""Permet de générer les fichiers train.fr,train.en,test.fr,test.en,dev.en et dev.fr
 input : le folder ted_data avec ses fichiers"""
...
class MultiLingualAlignedCorpusReader(object):
    ...
    def save_file(self, path_, split_type, data_type):
        with open(path_, 'w',encoding="utf8") as fp:
    ...
    def read_from_single_file(self, path_, s_lang, t_lang):
        data_dict = defaultdict(list)
        with open(path_, 'r',encoding='utf8') as fp:
...
if __name__ == "__main__":
    ...
    obj = MultiLingualAlignedCorpusReader(corpus_path=ted_data_path,lang_dict=train_lang_dict,target_token=True,corpus_type='file',eval_lang_dict=eval_lang_dict,zero_shot=False,
                                          bilingual=True)

    os.makedirs(output_data_path, exist_ok=True)
    obj.save_file(output_data_path + "/train.{}".format(src_lang),split_type='train', data_type='source')
    obj.save_file(output_data_path + "/train.{}".format(trg_lang),split_type='train', data_type='target')

    obj.save_file(output_data_path + "/test.{}".format(src_lang),split_type='test', data_type='source')
    obj.save_file(output_data_path + "/test.{}".format(trg_lang),split_type='test', data_type='target')

    obj.save_file(output_data_path + "/dev.{}".format(src_lang),split_type='dev', data_type='source')
    obj.save_file(output_data_path + "/dev.{}".format(trg_lang),split_type='dev', data_type='target')
